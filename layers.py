...
class prm_generator_laststage(nn.Module):
    def __init__(self, in_channel=64, norm="in", num_cls=4):
        ...
        self.embedding_layer = nn.Sequential(
            general_conv3d(
                in_channel * 4, int(in_channel // 4), k_size=1, padding=0, stride=1
            ),
            general_conv3d(
                int(in_channel // 4),
                int(in_channel // 4),
            ...
            general_conv3d(
                int(in_channel // 4), in_channel, k_size=1, padding=0, stride=1
            ),
        )
...
class region_aware_modal_fusion(nn.Module):
    def __init__(self, in_channel=64, norm="in", num_cls=4):
        ...
        self.modal_fusion = nn.ModuleList(
            [modal_fusion(in_channel=in_channel) for i in range(num_cls)]
        )
