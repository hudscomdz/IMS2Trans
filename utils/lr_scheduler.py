...
class LR_Scheduler(object):
    ...
    def __call__(self, optimizer, epoch):
        if self.mode == "poly":
            now_lr = round(
                self.lr
                * np.power(1 - np.float32(epoch) / np.float32(self.num_epochs), 0.9),
                8,
            )
...
def get_params(model):
    ignore_id = (
        list(
            map(
                id,
                model.module.decoder_all.abstraction1.fusion_conv.attention.parameters(),
            )
        )
        + list(
            map(
                id,
                model.module.decoder_all.abstraction2.fusion_conv.attention.parameters(),
            )
        )
        + list(
            map(
                id,
                model.module.decoder_all.abstraction3.fusion_conv.attention.parameters(),
            )
        )
        + list(
            map(
                id,
                model.module.decoder_all.abstraction4.fusion_conv.attention.parameters(),
            )
        )
    )
...
def record_loss(
    args, writer, mask1, loss_list, loss_name, step, mask_list, name_list, p_type
):
        ...
        for j in range(15):
            if torch.equal(mask1[i].int(), mask_list[j].int()):
                for k in range(len(loss_list)):
                    writer.add_scalar(
                        p_type[i] + "_" + name_list[j] + "_" + loss_name[k],
                        loss_list[k][i].item(),
                        global_step=step,
                    )
        ...
        for j in range(4):
            ...
            for k in range(K):
                for k1 in range(k + 1, K):
                    mutual_loss[i] += Js_div(
                        feat[:, k, :, :, :, :], feat[:, k1, :, :, :, :], KLDivLoss
                    )
